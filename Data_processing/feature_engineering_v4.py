"""
Feature Engineering V4 — Prédiction de blessures NBA
Fenêtre de prédiction : 10 jours

Corrections v4 :
    1. injury_next_10d  : exclure les joueurs déjà Out au moment du match
                          → on prédit une NOUVELLE blessure, pas la continuation d'une blessure
    2. b2b_10d          : vraie définition back-to-back (match la veille exacte)
    3. ACWR             : ajout charge 28j + ratio acute/chronic workload (7j/28j)
    4. days_out_season  : renommage de injury_count_season (c'était des jours, pas des épisodes)

Lancement :
    python feature_engineering_v4.py

Output : Datasets/nba_features_v4.parquet
"""
...
WINDOW_DAYS   = 10
CHRONIC_DAYS  = 28   # fenêtre charge chronique pour ACWR
OUTPUT_PATH   = Path("Datasets/nba_features_v4.parquet")
...
def compute_injury_durations(inj: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  Épisodes détectés : {len(df_ep):,} | Durée max : {df_ep['duration_days'].max()}j | Moyenne : {df_ep['duration_days'].mean():.1f}j")
    return df_ep
...
def build_rolling_features(logs: pd.DataFrame, inj: pd.DataFrame, episodes: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, (player_id, group) in enumerate(logs.groupby("PLAYER_ID")):
        ...
        for i, row in group.iterrows():
            ...
            # Charge aiguë = minutes moyennes par jour sur 7j
            # Charge chronique = minutes moyennes par jour sur 28j
            window_7d     = group[
            ...
            # ── CORRECTION 2 : vrai back-to-back ──
            # Un vrai b2b = match joué exactement la veille (J-1)
            if i > 0:
            ...
            # CORRECTION 4 : renommage days_out_season (c'est bien des jours, pas des épisodes)
            ...
            features = {
                ...
                "days_out_season"        : days_out_season,   # CORRECTION 4 : renommé
...
# ── Construction de la cible (CORRECTION 1) ───────────────────────────────────

def build_target(df_features: pd.DataFrame, inj: pd.DataFrame) -> pd.DataFrame:
    """
    CORRECTION v4 : on prédit une NOUVELLE blessure.
    Si le joueur est déjà Out au moment du match → on ne peut pas prédire
    une nouvelle blessure → on exclut ces lignes de la cible (NaN).

    Concrètement :
        - Si le joueur est Out à current_date → injury_next_10d = NaN (exclu)
        - Sinon → 1 si Out dans les 10 prochains jours, 0 sinon
    """
    print("\nConstruction de la cible (v4 — nouvelle blessure uniquement)...")

    inj_out = inj[inj["current_status"] == "Out"][["name_key", "game_date"]].copy()

    # Index des dates Out par joueur pour accès rapide
    out_index = {}
    for name_key, group in inj_out.groupby("name_key"):
    ...
    for _, row in df_features.iterrows():
        ...
        # CORRECTION 1 : joueur déjà Out aujourd'hui → exclure
        if current_date in player_out_dates:
            targets.append(np.nan)
            excluded += 1
            continue

        # Chercher une blessure Out dans les 10 prochains jours
        future_out = any(
            future_start <= d <= future_end
            for d in player_out_dates
        )
    ...
    print(f"  Lignes exclues (déjà Out)  : {excluded:,} ({excluded/len(df_features)*100:.1f}%)")
    print(f"  Lignes valides             : {valid.sum():,}")
...
def main():
    print("=" * 55)
    print("  Feature Engineering v4 — Prédiction blessures NBA")
    print("=" * 55)
    ...
    # Supprimer les lignes exclues (joueurs déjà Out)
    ...
    print(f"\n  ACWR :")
    print(f"    acwr : min={df_features['acwr'].min():.2f} max={df_features['acwr'].max():.2f} mean={df_features['acwr'].mean():.2f}")
    print(f"    b2b  : {df_features['b2b'].value_counts().to_dict()}")
    print(f"\n  Split :")
